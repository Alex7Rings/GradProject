...
from crud.crud import (
    ...
    get_trades, bulk_upsert_trades, parse_trades_file, get_portfolio_trades_grouped,
    bulk_upsert_historical_prices, get_historical_prices, delete_historical_prices,
    delete_historical_data_by_ticker, fetch_yahoo_historical,
    get_ticker_returns, get_portfolio_weighted_returns, get_portfolio_covariance,
    get_portfolio_weights, train_lstm_model, predict_lstm_returns,
    get_historical_var, backtest_historical_var, get_parametric_var, parse_trade_data
)
...
def upload_trades_json_xml(data: Dict[str, Any], db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not current_user.portfolio:
        raise HTTPException(status_code=400, detail="No portfolio found")
...
def get_grouped_trades_endpoint(group_by: str = Query("instrument_type"), db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
def fetch_yahoo_endpoint(ticker: str = Path(...), db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    try:
...
def delete_historical_data_endpoint(ticker: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not current_user.portfolio:
        raise HTTPException(status_code=400, detail="No portfolio found")
...
# Portfolio and Analysis Endpoints
@router.get("/users/me/returns/{ticker}", response_model=List[float])
def get_ticker_returns_endpoint(ticker: str = Path(...), db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not current_user.portfolio:
        raise HTTPException(status_code=400, detail="No portfolio found")
    try:
        ticker = ticker.replace("_", "/")
...
def get_portfolio_returns_endpoint(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not current_user.portfolio:
        raise HTTPException(status_code=400, detail="No portfolio found")
    try:
...
def get_portfolio_weights_endpoint(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not current_user.portfolio:
        raise HTTPException(status_code=400, detail="No portfolio found")
    try:
...
def get_covariance_matrix(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not current_user.portfolio:
        raise HTTPException(status_code=400, detail="No portfolio found")
    try:
...
def get_historical_var_endpoint(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not current_user.portfolio:
        raise HTTPException(status_code=400, detail="No portfolio found")
    try:
...
def backtest_historical_var_endpoint(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not current_user.portfolio:
        raise HTTPException(status_code=400, detail="No portfolio found")
    try:
...
def get_parametric_var_endpoint(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not current_user.portfolio:
        raise HTTPException(status_code=400, detail="No portfolio found")
    try:
...
def train_lstm_endpoint(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not current_user.portfolio:
        raise HTTPException(status_code=400, detail="No portfolio found")
    try:
